...
def search_node(seq, path=[]):
    for i, node in enumerate(seq):
        if node.get('name') == 'Set topic':
            ...
            if i > 0:
                out.append('Prev: ' + str(seq[i-1].get('name')) + ' | ' + str(seq[i-1].get('properties')))
            out.append('Curr: ' + str(node.get('name')) + ' | ' + str(node.get('properties')))
            if i < len(seq) - 1:
                out.append('Next: ' + str(seq[i+1].get('name')) + ' | ' + str(seq[i+1].get('properties')))
...
with open(r'C:\Users\Administrator\Desktop\Hoc_python\scratch\inspect_set_topic.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(out))
