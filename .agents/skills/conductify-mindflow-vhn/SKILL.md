...
## 21. KIẾN THỨC NỀN TỪ `AI_Trainer_BaoCao_TongHop_2.docx`

Nguồn cập nhật: `C:\Users\Administrator\Desktop\Hoc_python\AI_Trainer_BaoCao_TongHop_2.docx` (Neurons AI Trainer, Conductify AI Studio, các buổi 3/4/6/9/11 và phần reference Mindflow/Knowledge Bank).

### 21.1. Mô hình tư duy Conductify AI Studio

- Conductify AI Studio dùng để tạo AI Agent vượt qua chatbot hỏi-đáp: có thể phân luồng, phân tích, ra quyết định, gọi API, cập nhật thông tin và tích hợp đa kênh.
- Khu vực chính của Agent gồm: `Mindflow` (thiết kế logic), `Playground` (test), `Agent Profile` (tên/avatar/mô tả), `Agent Settings` (chat history, starting questions).
- Luồng cơ bản: User message -> Session Manager -> Router -> Flows Switch -> Flow xử lý -> Clear Topic.
- Flowchart là bản phác thảo trước khi build Mindflow. Một flow trong flowchart KHÔNG bắt buộc ánh xạ 1:1 sang Mindflow; nếu kiến thức/logic quá lớn thì tách thành nhiều flow nhỏ, mỗi flow có AI QnA chuyên trách riêng.

### 21.2. Knowledge Bank và chiến lược dữ liệu

- Dữ liệu chất lượng cao là điều kiện bắt buộc để AI đại diện doanh nghiệp; thiếu dữ liệu thì AI dễ "chế" thông tin.
- `General Knowledge` phù hợp nội dung dài, cố định, ít thay đổi (file/text/url, book theo chủ đề). Độ ưu tiên thấp hơn QnA.
- `Questions & Answers` phù hợp thông tin nhỏ, cụ thể, thay đổi theo thời gian (sản phẩm, giá, liên hệ, chính sách). AI ưu tiên trả lời sát QnA hơn General Knowledge.
- Với bộ QnA lớn, ưu tiên import/export CSV với 2 cột `question` và `answer`.
- Quick Test phải được dùng sau khi nạp data để kiểm tra semantic search và số kết quả truy xuất.
- Content Splitter cho General Knowledge: `Markdown` cắt theo heading/code/separator/dòng trống; `Paragraph` cắt theo đoạn; `Token` cắt theo token với segment length và overlap.
- Khi bật Knowledge Aware trong AI QnA, phải điền đủ `Knowledge Dataset UUID` và `Knowledge Dataset QA UUID`; nếu thiếu UUID AI có thể không truy xuất dữ liệu và trả lời sai.
- Khuyến nghị từ tài liệu: Knowledge Result Limit khoảng 3-5 cho Book và 2-3 cho QnA. Khi tối ưu token, bắt đầu bằng limit thấp và tăng dần theo test.

### 21.3. AI QnA, model, temperature và token

- AI QnA là "AI chuyên trách" cho từng nhiệm vụ cụ thể; prompt là phần quyết định hành vi quan trọng nhất.
- `Enable Human Input Aware` cho phép đọc tin nhắn mới nhất; `Enable Chat History Aware` cho phép đọc lịch sử hội thoại. Chỉ bật khi node thật sự cần ngữ cảnh.
- `Variable to save AI reply` dùng để lưu output và gọi lại bằng `{{ ten_bien }}`.
- JSON Mode chỉ dùng được với các model GPT của OpenAI. Khi AI trả JSON, nên có node parse/guard để strip markdown code fence trước `json.loads`.
- Temperature: `<= 0.6` cho task cần chính xác/extract/order; `0.7` cân bằng; `>= 0.8` dễ sáng tạo/bịa; `>= 1.0` không khuyến khích cho sales/medical/financial.
- Max Tokens mặc định khoảng 1000; không để quá cao nếu node chỉ cần phân loại/JSON ngắn. Node Manager/Router nên output ngắn.

### 21.4. Dữ liệu, biến và block lập trình

- `Constants` dùng cho dữ liệu không đổi, gọi bằng `{{ten_const}}`.
- `Variables` dùng cho dữ liệu thay đổi trong lúc chạy, gọi bằng `{{ten_bien}}`.
- System variables quan trọng: `human_input`, `date_time`, `chat_history`, `bot_uuid`, `channel_type`, `channel_id`, `thread_id`, `user_msg_id`, `attachments`, `session`.
- Session extra gọi bằng `{{ session.extra.ten_bien }}`; chỉ nên lưu biến thay đổi theo hội thoại. Dữ liệu tĩnh/lớn nên đưa vào Knowledge Bank hoặc prompt chuyên trách thay vì nhồi vào session.
- Built-in functions hay dùng: `empty_str()`, `to_str(variable)`, `to_num(s)`, `booleanize(var)`, `check_dict_with_key(var, key)`, `is_none_or_empty(var)`, `json.loads(s)`, `json.dumps()`.
- `Python Expression` dùng cho biểu thức ngắn có return value; `Python` block dùng cho code phức tạp hơn, kết quả `print()` được lưu vào biến.
- `Break` dùng để debug và dừng các block phía sau.

### 21.5. Branching, Parallel, Set Topic và Recall

- Theo tài liệu đào tạo, `If` rẽ nhánh true/false; `Parallel` tương tự If nhưng có nhiều nhánh, mỗi nhánh có điều kiện riêng.
- Trong thực chiến VHN/Shegan, cần phân biệt:
  - `Parallel` có thể tạo fan-out nếu nhiều điều kiện đúng cùng lúc, làm tốn token và gọi nhiều node. Dùng nó như multi-branch có điều kiện loại trừ nhau rõ ràng; nếu không chắc, dùng if-chain hoặc flowSwitch action phẳng.
  - Tài liệu đào tạo có nhắc pattern `Set Topic + Recall` để đổi topic. Tuy nhiên raw JSON/reference Shegan hiện tại không xác nhận component `setTopic` ổn định. Khi sửa raw JSON, KHÔNG tự thêm `setTopic` nếu chưa thấy trong file mẫu đang chạy; ưu tiên `botSendText`/`clearTopic` để lượt sau Router phân lại, hoặc giữ pattern đã được file mẫu xác thực.
  - Nếu dùng `Recall`, không đặt `Clear Topic` trong cùng luồng Recall; recall properties trong raw JSON nên để rỗng `{}` theo rule thực chiến.

### 21.6. AI Sales Agent: thiết kế tổng thể

- Mục tiêu AI Sales Agent: cung cấp thông tin doanh nghiệp/chính sách/khuyến mãi, tư vấn sản phẩm theo nhu cầu, upsell/cross-sell, hỗ trợ đặt hàng, ghi nhận đơn hàng vào hệ thống nội bộ, tạo ticket/handover khi không xử lý được.
- Nhóm flow chính: hỏi đáp thông tin; handover/gọi hỗ trợ; tư vấn sản phẩm; đặt hàng/order.
- Flow hỏi đáp thông tin thường là Basic: AI QnA + Knowledge Dataset + Clear Topic.
- Handover nên dùng AI phân tích JSON, parse biến, gọi API tạo ticket nếu có endpoint.
- Tư vấn sản phẩm nên theo pipeline: AI extract nhu cầu -> Python check đủ biến -> If/Recall hỏi tiếp nếu thiếu -> Bot API Call/AI Vector DB -> dùng kết quả API trả lời -> lưu chat history/session -> lưu sản phẩm quan tâm cho order.
- Order nên dùng thông tin sản phẩm đã lưu từ consult để truy vấn lại DB, tính giá/chính sách, hỏi thông tin cá nhân, ghi đơn qua API, xóa lịch sử đơn cũ và lưu thông tin chăm sóc sau.
- Lưu ý từ tài liệu Shegan: trong "AI Tổng quản" của order, có trường hợp KHÔNG bật Chat History Aware và Human Input Aware để tránh nhớ đơn hàng cũ. Khi áp dụng, phải kiểm tra mục tiêu node: node nào cần ngữ cảnh thì bật, node nào chỉ phân tích đơn hiện tại thì hạn chế history.

### 21.7. API, Contact List và tích hợp đa kênh

- Bot API Call cần `URL`, `Method` (`GET/POST/PUT/DELETE`) và Raw Body JSON nếu cần; nên test bằng Postman trước khi đưa vào Mindflow.
- Kết quả API gọi bằng `{{variable_name_api.ten_bien}}`.
- API Contact List tham chiếu: `POST https://api.workerbot.ai/v1/contact-list/create`.
- Body có thể gồm `email`, `name`, `phone`, `address`, `company`, `bot_uuid`, `thread_id`, và `full_field` để lưu thêm `product_requirement`, `special_requirement`, ...
- `bot_uuid` và `thread_id` là system variables lấy trực tiếp từ Mindflow.
- Integration Browser dùng để đưa Agent lên Zalo/Facebook; AI CRM dùng để quản lý hội thoại đa kênh, manual takeover, sửa Q&A, xem integration, contact list và export CSV.

### 21.8. Quy trình làm việc chuẩn cho AI Trainer

1. Xác định mục tiêu Agent, phạm vi, đầu vào/đầu ra và các bên liên quan.
2. Vẽ flowchart trước: tách flow theo mục tiêu nghiệp vụ, xử lý tuần tự từng flow và test xong mới mở rộng.
3. Chuẩn bị dữ liệu: chọn nguồn uy tín, tách General Knowledge/QnA, làm sạch định dạng, bổ sung thông tin từ hình/bảng vì Conductify chỉ trích text.
4. Build Mindflow: Session -> Router -> Flows Switch -> các AI QnA/block logic -> Filter/Bot Send Text -> Clear Topic.
5. Dùng Playground + Show Raw Output để xem prompt thực tế, phát hiện context quá lớn hoặc biến bị chèn sai.
6. Test Quick Test cho Knowledge Bank, test từng flow riêng, sau đó test end-to-end theo kịch bản sales.
7. Khi tối ưu token, ưu tiên cắt context ở Manager/Router, giảm Knowledge Result Limit, tắt aware flags không cần, giảm max_tokens của node phân tích, và chỉ đưa dữ liệu lớn vào node chuyên trách.
